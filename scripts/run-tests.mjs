 ...
 * directory on disk: `data/seed/mutable/`. The mock-env suite clears it to
 * prove a reset works, which would delete a session another suite had just
 * written and was about to read back. It failed roughly one run in twenty,
 * which is the worst possible rate: often enough to matter, rarely enough to
 * be dismissed as a fluke.
 *
 * The whole suite takes well under a second, so serialising costs nothing.
 */
