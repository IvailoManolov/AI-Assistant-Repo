...
Suites run one at a time. They share `data/seed/mutable/`, and the mock-env
suite clears it to prove a reset works, so two of them running at once could
delete a session another was about to read back. The whole suite takes under a
second, so serialising costs nothing.
