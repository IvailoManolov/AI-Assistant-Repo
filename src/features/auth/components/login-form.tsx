    ...
    <>
      ...
      <div className="mt-7 border-t border-line pt-5">
        <button
          ...
          className="text-[13px] font-medium text-ink-soft transition-colors hover:text-coral-deep"
        >
          {manual ? "Hide the typed sign-in" : "Type a username and password instead"}
        </button>
        ...
        {manual && (
          <form
            ...
            className="pane-in mt-4 grid gap-3.5"
            noValidate
          >
            ...
            <button
              type="submit"
              className="mt-1 rounded-full bg-ink px-6 py-3 text-sm font-semibold text-cream transition-colors hover:bg-coral-deep"
            >
              Sign in
            </button>
          </form>
      ...
      <p className="mt-6 text-[13px] leading-relaxed text-muted">
        There is no database and no account store. The two logins exist to switch roles, which is
        why their passwords are printed on the cards.
      </p>
    </>
  );
}
